...
class ReviewKD(Distiller):
    ...
    def __init__(
        ...
        self.teacher_features_shapes, self.student_features_shapes = get_feature_shapes(
            teacher=self.teacher, student=self.student, input_size=input_size, pooling=True
        )
    ...
    def forward_train(self, image, target, **kwargs):
        ...
        features_teacher = teacher_feature["features"][1:] + [
            teacher_feature["avgpool_feature"].unsqueeze(-1).unsqueeze(-1)
        ]

        # Compute loss
        loss_ce = self.ce_weight * F.cross_entropy(logits_student, target)

        loss_reviewkd = self.reviewkd_weight * hcl_loss(results, features_teacher)

        loss_dict = {"loss_ce": loss_ce, "loss_reviewkd": loss_reviewkd}
