...
def shear_x(probability: float, magnitude: int) -> OperationBase:
    ...
    return ShearX(
        None,
        probability,
        magnitude_range=p((magnitudes[magnitude].item(), magnitudes[magnitude + 1].item())),
...
def posterize(probability: float, magnitude: int) -> OperationBase:
    ...
    return Posterize(None, probability, magnitude_range=(magnitudes[magnitude].item(), magnitudes[magnitude + 1].item()))


def contrast(probability: float, magnitude: int) -> OperationBase:
    magnitudes = linspace(0.1, 1.9, 11)
    return Contrast(None, probability, magnitude_range=p((magnitudes[magnitude].item(), magnitudes[magnitude + 1].item())))


def brightness(probability: float, magnitude: int) -> OperationBase:
    magnitudes = linspace(0.1, 1.9, 11)
    return Brightness(
        None, probability, magnitude_range=p((magnitudes[magnitude].item(), magnitudes[magnitude + 1].item()))
    )


def sharpness(probability: float, magnitude: int) -> OperationBase:
    magnitudes = linspace(0.1, 1.9, 11)
    return Sharpness(None, probability, magnitude_range=p((magnitudes[magnitude].item(), magnitudes[magnitude + 1].item())))


def color(probability: float, magnitude: int) -> OperationBase:
    magnitudes = linspace(0.1, 1.9, 11)
    return Saturate(None, probability, magnitude_range=p((magnitudes[magnitude].item(), magnitudes[magnitude + 1].item())))