...
def ab_loss(teacher_feature, student_feature, margin):
    ...
    loss = (student_feature + margin) ** 2 * ((student_feature > -margin) & (teacher_feature <= 0)).float() + (
        student_feature - margin
    ) ** 2 * ((student_feature <= margin) & (teacher_feature > 0)).float()
...
class FullyConnectorAB(Distiller):
    def __init__(self, teacher, student, ce_weight=1.0, ab_weight=1.0, margin=1.0, combined_KD=False, temperature=None):
    ...
    def forward_train(self, image, target, **kwargs):
        ...
        if (
            ...
            and teacher_featuremap.shape[2] * teacher_featuremap.shape[3]
            != student_featuremap.shape[2] * student_featuremap.shape[3]
        ):
            self.linear_layer = nn.Linear(
                student_featuremap.shape[2] * student_featuremap.shape[3],
                teacher_featuremap.shape[2] * teacher_featuremap.shape[3],
            ).to("cuda")
        ...
        loss_ab = self.ab_weight * ab_loss(
            teacher_feature=teacher_featuremap, student_feature=student_featuremap, margin=self.margin
        )

        loss_dict = {"loss_ce": loss_ce, "loss_ab": loss_ab}
...
class ConvolutionAB(Distiller):
    ...
    def forward_train(self, image, target, **kwargs):
        ...
        if (
            teacher_featuremap.shape[2] * teacher_featuremap.shape[3]
            == student_featuremap.shape[2] * student_featuremap.shape[3]
            and teacher_featuremap.shape[1] != student_featuremap.shape[1]
        ):
            self.conv_layer = nn.Conv2d(student_featuremap.shape[1], teacher_featuremap.shape[1], kernel_size=(1, 1)).to(
                "cuda"
            )
